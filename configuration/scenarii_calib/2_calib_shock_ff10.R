...
shock_ch <- if(classification == "c8_s8"){mutate(selection, 
                                                pwd_cend =   ifelse(year >= shockyear, pwd_cend * (1 + 0.1), pwd_cend) #replace 0.2 by oil and gas part in "cend" (energy dirty commodity) - search
)
...
}else{if(classification == "c29_s33"){mutate(selection, 
