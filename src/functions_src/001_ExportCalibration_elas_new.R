...
ExportCalibration_elas   <- function(iso3, names_sectors) {
  ...
  calib <-readxl::read_xlsx(str_c("data/input/France/DATA_ELAS_NEW.xlsx"), sheet = "ELAS", col_names = TRUE, range = "A2:G35") %>% 
  ...
  writeTextFile(exported, str_c("src/model/threeme/data/R_ExtraCalibration_elas_KLEM_", toupper(iso3), "_c29_s33.mdl"))
}
