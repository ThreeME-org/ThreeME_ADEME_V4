...
#' @param check_digit (1), nom du ou des scénarios
#' @param template_default character(1) string, nom du thème ggplot retenu pour le plot
#' @return un dataframe
#' @import data
#' @family function
#' @export
contrib.sub_country <- function(data, var1,  group_type = "sector",
                            country = NULL,
                            ...
                            check_digit = NULL)
{
  ...
  liste_var <- var_vec[grep(paste0("^",var1,"_",group,"[A-Z0-9]{3}","_",country,"$"),var_vec)]
  var1 <- paste0(var1,"_",country)
  filtered.var <-  filter(data,variable %in% liste_var)
  ...
  data.contrib.0 <- data %>% filter(variable %in% c(var1, filtered.val)) #%>%
  ...
  data.contrib.lbl <- data %>% filter(variable %in% c(var1, filtered.val)) %>%
