---
title: "`r trad('Standard shock simulation')`"
author: "ThreeME team"
...
output:
  slidy_presentation: 
    ...
    toc_depth: 2
subtitle: "`r countrycode(iso3, origin = 'iso3c', destination = 'country.name')`"
params:
  startyear: 2020
  endyear: 2050
...
path_translation_file <- file.path(work.dir,"/results/report/markdown/src/translation_file.csv")
...
output <- readRDS(file = file.path("data","output","shock_simulation.rds"))
...
#Define palettes
if(exists("color_theme")==FALSE){color_theme = "SAND"}
...
flextable::flextable(data = flextable_data_prodk10) %>%
add_header_row(values = c(trad("ThreeME model"),paste(trad("Standard shock:"),str_to_upper(name))),colwidths = c(1,7)) %>%
add_footer_lines(footnote) %>% 
flextable_theme()
#%>% color(i = ~Variable %in% "Imports",j= c(3:8),color = "#eb5555")
...
```{r Table macro baseline, results ='asis'}
name = "baseline"

flextable::flextable(data = flextable_data_baseline) %>%
...
B <- ggplot(data_g1, aes(x = year, y = variation)) + 
...
A <- plotly::ggplotly(B, tooltip = "text", dynamicTicks = FALSE,
               width = 875,
               height = 650
               ) %>% 
...
B <- ggplot(data_g, aes(x = year, y = variation)) + 
  geom_line(aes(color=label,group = 1,text = text),size = 0.5) +
  scale_colour_manual(values = lines_palette) + 
  labs(x="",y=paste0("% (",trad("in difference from baseline"),")"),color="") + 
  theme(text = element_text(family = 'Arial',size = 12),
